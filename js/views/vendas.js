...
import { listarVendas, criarVenda, cancelarVenda, listarClientes, listarCategorias, listarProdutos, listarItensVenda } from "../api.js";
...
function item(v) {
  ...
  if (!cancelada) {
    const bRecibo = el("button", { class: "btn btn--tiny btn--ghost" }, "Comprovante");
    bRecibo.addEventListener("click", async () => {
      bRecibo.disabled = true;
      try {
        // Venda de valor manual não tem itens — o recibo cai na descrição.
        const itens = await listarItensVenda(v.id).catch(() => []);
        imprimirRecibo({
          numero: v.id.slice(0, 8).toUpperCase(),
          data: v.occurred_on,
          empresa: state.company.name,
          empresaDoc: state.company.doc,
          cliente: v.parties?.name,
          clienteDoc: v.parties?.doc,
          descricao: v.description,
          totalCents: v.amount_cents,
          itens,
        });
      } finally { bRecibo.disabled = false; }
    });
    acoes.append(bRecibo);
